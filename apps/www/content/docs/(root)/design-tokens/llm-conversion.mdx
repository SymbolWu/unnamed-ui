...
| `font-size` | `font-size-2`（预定义类）或 `text-[length:var(--font-size-2)]`，**不要**用 `text-[var(--font-size-2)]`（易与颜色混淆） |
...
**字号与行高：**
- `font-size-1`～`font-size-6`：预定义类（12px～28px），或 `text-[length:var(--font-size-2)]`
