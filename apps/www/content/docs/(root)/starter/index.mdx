...
1. [环境准备与安装](#1-环境准备与安装)
2. [自定义 Token（主题定制）](#2-自定义-token主题定制)
3. [实战演练：搭建 Proposal 页面](#3-实战演练搭建-proposal-页面)
4. [核心功能：对话与流式输出](#4-核心功能对话与流式输出)
5. [UWS 组件](#5-uws-组件-uws-components)
...
组件库通过 CSS 变量（如 `--brand-600`、`--radius`）控制品牌色、圆角等视觉表现。若直接修改 `globals.css`，每次执行 `shadcn add` 安装新组件时，可能被覆盖。建议使用**独立主题文件**，将自定义 token 与组件库样式分离。

### 2.1 新建主题文件

在 `src/` 下新建 `theme.css`（或 `tokens.css`），仅包含需要覆盖的变量：

```css
/* 自定义 Token - 此文件不会被 shadcn 覆盖 */
:root {
  /* 品牌色 - 主色常用于按钮、链接、焦点环 */
  --brand-50: #faf7f5;
  --brand-100: #ffebde;
...
> 只需覆盖需要修改的变量，未覆盖的将使用组件库默认值。常用变量：`--brand-*`（品牌色）、`--radius*`（圆角）、`--neutral-*`（中性色）、`--font-sans`（字体）。

### 2.2 在入口最后引入

在 `src/index.css`（Vite）或 `app/globals.css`（Next.js）**末尾**添加：
...
```tsx
// main.tsx 或 layout.tsx
import "./index.css";
import "./theme.css";  // 后导入的变量会覆盖前面的
```

### 2.3 为何不会被覆盖？

- `shadcn add` 只会更新 `components.json` 中配置的样式文件（如 `globals.css`）
- `theme.css` 是您自行创建的文件，不在 shadcn 的更新范围内
- 由于在入口中**最后**引入，其中的变量会覆盖组件库的默认值

---

## 3. 实战演练：搭建 Proposal 页面
