...
如果想一次性安装所有组件（共 54 个），可以使用批量安装脚本：
...
# 方式 2: 一行命令直接安装
curl -fsSL https://raw.githubusercontent.com/your-username/unnamed-ui/main/apps/www/scripts/install/install-components.cjs | node - https://unnamed-ui.vercel.app
```

批量安装包含所有 54 个组件，约需 3-5 分钟。安装过程会自动覆盖已存在的文件，无需手动确认。

> 如果只想安装特定组件，可以跳过此步骤，在需要时按照后续章节的说明逐个安装。
